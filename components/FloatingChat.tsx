...
export default function FloatingChat({ whatsappNumber = "6281234567890" }: FloatingChatProps) {
  ...
  // Hide customer support chat on admin backend routes (after all hooks are called)
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    // 2. Automated agent response with WA button
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        {
      ...
      >
        ...
        <div className="border-b border-[#1f1f1f] p-4 flex justify-between items-center bg-[#111111]">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-[#ececec] text-xs uppercase tracking-widest font-bold">
              Mantra Support
            </span>
          </div>
          <button
            ...
            className="text-[#ececec]/60 hover:text-white transition-colors cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {!session?.user ? (
          ...
          <>
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-4 flex flex-col scrollbar-hide">
              {messages.map((msg, i) => (
                <div
                  key={i}
                  className={cn(
                    "max-w-[85%] p-3 text-sm flex flex-col gap-2",
                    msg.role === "agent"
                      ? "bg-[#1f1f1f] text-[#ececec] self-start"
                      : "bg-[#ececec] text-[#050505] self-end"
                  )}
                >
                  <span>{msg.text}</span>
                  ...
                  {msg.showWaButton && (
                    <a
                      href={`https://wa.me/${waNumber}?text=${encodeURIComponent(
                        `Hello MANTRA Admin, I would like to ask: "${msg.userMessage}"`
                      )}`}
                      target="_blank"
            ...
            <form onSubmit={handleSend} className="border-t border-[#1f1f1f] p-2 flex bg-[#111111]">
              ...
              <button
                type="submit"
                className="p-2 text-[#ececec]/60 hover:text-white cursor-pointer"
              >
