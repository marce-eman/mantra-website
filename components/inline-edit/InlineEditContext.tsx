...
import { Eye, Edit3, CheckCircle2, AlertCircle, X, Sparkles } from "lucide-react";
...
export function InlineEditProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      {/* Floating Admin Live CMS Indicator Bar */}
      {isAdmin && (
        <div className="fixed bottom-6 left-6 z-[9999] flex flex-col items-start gap-2 pointer-events-auto select-none">
          <div className="flex items-center gap-2.5 bg-[#0a0a0a]/95 backdrop-blur-xl border border-pink-500/40 px-4 py-2 rounded-full shadow-[0_0_25px_rgba(236,72,153,0.25)] text-xs font-mono transition-all">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] uppercase tracking-wider font-semibold transition-all cursor-pointer ${
                isEditMode
            ...
            >
              {isEditMode ? (
                <>
                  <Edit3 className="w-3 h-3 text-pink-400" /> EDIT MODE: ON
                </>
              ) : (
                <>
                  <Eye className="w-3 h-3 text-gray-400" /> PREVIEW MODE
                </>
              )}
            </button>
            ...
            <Link
              href="/admin"
              className="text-[#ececec]/60 hover:text-pink-400 text-[9px] uppercase tracking-wider transition-colors px-1"
            >
              DASHBOARD &rarr;
            </Link>
          </div>
