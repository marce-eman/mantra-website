...
export default function Navbar() {
  ...
  const activeStyle =
    "px-5 py-1.5 border border-[#1f1f1f] rounded-full text-xs uppercase tracking-widest text-[#ececec] bg-[#111111] transition-all";
  const inactiveStyle =
    "px-5 py-1.5 text-xs uppercase tracking-widest text-[#ececec]/60 hover:text-white transition-colors";

  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-[#050505]/75 backdrop-blur-md border-b border-[#1f1f1f] transition-all duration-300">
      <div className="max-w-screen-2xl mx-auto px-4 h-16 flex items-center justify-between">

        {/* Left Side: Home + Episodes */}
        <div className="flex-1 flex items-center justify-end space-x-4 md:space-x-8 pr-8 md:pr-12">
          <Link
            href="/"
            onClick={() => handleNavClick("home")}
            className={pathname === "/" && activeTab === "home" ? activeStyle : inactiveStyle}
          >
            Home
          </Link>
          <Link
            ...
            className={`${
              pathname === "/" && activeTab === "collection" ? activeStyle : inactiveStyle
            } hidden sm:block`}
          >
            Episodes
          </Link>
        ...
        <div className="flex-1 flex items-center justify-start space-x-4 md:space-x-8 pl-8 md:pl-12">
          ...
          <button
            ...
            className={`flex items-center space-x-2 ${inactiveStyle} cursor-pointer`}
          >
